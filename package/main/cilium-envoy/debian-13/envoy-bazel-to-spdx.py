...
Usage:
    python envoy-bazel-to-spdx.py $name $version [bazel aquery options...]
...
def run_bazel_aquery(bazel_options):
    print(
...
def generate_spdx(linked_deps, all_repositories, name, version):
    ...
    document_id = uuid.uuid5(uuid.NAMESPACE_URL, f"{name}:{version}")
    spdx_doc = {
        "spdxVersion": "SPDX-2.3",
        "dataLicense": "CC0-1.0",
        "SPDXID": f"SPDXRef-{name}",
        "name": f"SPDX document for {name} {version}",
        "documentNamespace": f"https://spdx.org/spdxdocs/{name}-{version}-{document_id}",
        "creationInfo": {
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python envoy-bazel-to-spdx.py <name> <version> "
            "[bazel aquery options...]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    aquery_output = run_bazel_aquery(bazel_options)
