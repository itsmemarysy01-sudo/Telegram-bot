#!/bin/bash
# Shared time-mcp venv staging for alpine and debian.
# Expects (exported by the definition pipeline — melange vars are not
# expanded inside this file):
#   SOURCE_DIR  - work dir containing time/ (upstream time server tree)
...
SRC="${SOURCE_DIR}/time"
VENV="${TARGET_DIR}/usr/lib/time-mcp"
...
test -e "${TARGET_DIR}/usr/lib/time-mcp/bin/mcp-server-time"
ln -sf ../lib/time-mcp/bin/mcp-server-time "${TARGET_DIR}/usr/bin/mcp-server-time"
...
mkdir -p /opt/docker/sbom/time-mcp
