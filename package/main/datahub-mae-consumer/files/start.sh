...
if [[ ${ENABLE_OTEL:-false} == true ]]; then
  JAVA_TOOL_OPTIONS="$JAVA_TOOL_OPTIONS -javaagent:/usr/share/datahub-mae-consumer/lib/opentelemetry-javaagent.jar"
fi
if [[ ${ENABLE_PROMETHEUS:-false} == true ]]; then
  JAVA_TOOL_OPTIONS="$JAVA_TOOL_OPTIONS -javaagent:/usr/share/datahub-mae-consumer/lib/jmx_prometheus_javaagent.jar=4318:/usr/share/datahub-mae-consumer/scripts/prometheus-config.yaml"
fi

export JAVA_TOOL_OPTIONS
exec dockerize "${dockerize_args[@]}" java -jar /usr/share/datahub-mae-consumer/bin/mae-consumer-job.jar
