...
> [!NOTE] Bear in mind that if you want to use dynamic volume provisioning, you need to specify the
> `global.defaultStorageClass` as it's blanked out by default.

```console
helm install my-mongodb oci://dhi.io/mongodb-chart --version <version> \
...
That should get a MongoDB pod up and running.

```bash
$ kubectl get pods
NAME                                          READY    STATUS    RESTARTS   AGE
my-mongodb-mongodb-chart-5bfdc9976-bftdh   1/1     Running   0          11s
```

Port-forward the MongoDB service (default port 27017) and connect with a MongoDB client, for example `mongosh`, using
the credentials you configured in chart values.
