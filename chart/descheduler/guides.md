...
```console
regctl image copy   "${SRC_CHART_REPO}:${TAG}"   "${DEST_REG}/${DEST_CHART_REPO}:${TAG}"   --referrers   --referrers-src "${SRC_ATT_REPO}"   --referrers-tgt "${DEST_REG}/${DEST_CHART_REPO}"   --force-recursive
```
...
```console
kubectl create secret docker-registry helm-pull-secret   --docker-server=dhi.io   --docker-username=<Docker username>   --docker-password=<Docker token>   --docker-email=<Docker email>
```
...
```console
helm install my-descheduler oci://dhi.io/descheduler-chart --version <version>   --set "imagePullSecrets[0].name=helm-pull-secret"
```
...
The CronJob should be created in the release namespace:

```console
$ kubectl get cronjob
NAME                               SCHEDULE      TIMEZONE   SUSPEND   ACTIVE   LAST SCHEDULE   AGE
my-descheduler-descheduler-chart   */2 * * * *   <none>     False     0        <none>          30s
```
