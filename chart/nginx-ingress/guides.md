...
```console
helm install my-nginx-ingress oci://dhi.io/nginx-ingress-chart --version <version> \
  --set "controller.serviceAccount.imagePullSecretsName=helm-pull-secret" \
```
...
Note that this chart does not feature the NGINX Plus version and hence app protection with F5 WAF should not be enabled.
...
pod/my-nginx-ingress-controller-846444b6fb-lgdpd           1/1     Running   0          30s

NAME                                          TYPE           CLUSTER-IP     EXTERNAL-IP   PORT(S)                      AGE
service/kubernetes                            ClusterIP      10.43.0.1      <none>        443/TCP                      50s
service/my-nginx-ingress-controller           LoadBalancer   10.43.112.23   172.17.0.5    80:31935/TCP,443:31418/TCP   30s

NAME                                                  READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/my-nginx-ingress-controller           1/1     1            1           30s

NAME                                                             DESIRED   CURRENT   READY   AGE
replicaset.apps/my-nginx-ingress-controller-846444b6fb           1         1         1       30s
```
