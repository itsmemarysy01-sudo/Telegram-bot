...
- Istio installed in the cluster (Kiali is a console for an existing Istio service mesh)
...
```console
helm install my-kiali oci://dhi.io/kiali-server-chart --version <version> \
  --set "deployment.image_pull_secrets[0]=helm-pull-secret"
```
...
pod/kiali-5c4fc84bbf-kx9cd   1/1     Running   0          18s

NAME                TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)             AGE
service/kiali       ClusterIP   10.96.48.167   <none>        20001/TCP,9090/TCP  18s

NAME                    READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/kiali   1/1     1            1           18s
```

Note that, unlike most charts, the Kiali resources are named after `deployment.instance_name` (default: `kiali`) rather
than the Helm release name.
