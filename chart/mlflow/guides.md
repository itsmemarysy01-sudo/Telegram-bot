...
> **Note:** `mlflow.backendStoreUri` is a required value. It specifies where MLflow stores experiment and run metadata.
> For production use, point it to an external database (e.g. PostgreSQL). The example below uses SQLite, which is
> suitable for simple or single-node deployments.

```console
helm install my-mlflow oci://dhi.io/mlflow-chart --version <version> \
...
pod/test-mlflow-chart-565f95ffff-mx9sp   1/1     Running   0          53s

NAME                        TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)    AGE
service/kubernetes          ClusterIP   10.43.0.1       <none>        443/TCP    107s
service/test-mlflow-chart   ClusterIP   10.43.201.117   <none>        5000/TCP   53s

NAME                                READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/test-mlflow-chart   1/1     1            1           53s

NAME                                           DESIRED   CURRENT   READY   AGE
replicaset.apps/test-mlflow-chart-565f95ffff   1         1         1       53s
```
