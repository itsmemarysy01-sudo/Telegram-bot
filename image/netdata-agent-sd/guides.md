...
This image runs `agent_sd`, a Kubernetes service discovery component for the Netdata monitoring agent. It watches the
Kubernetes API server for Pod and Service events and automatically generates data collection configurations for Netdata
collectors.
...
Use `agent_sd` with `--help` to display usage information and validate that the image is working.

```
$ docker run --rm dhi.io/netdata-agent-sd:<tag> --help
```

Netdata Agent SD requires a configuration file or a Kubernetes ConfigMap to operate. It is typically deployed as a
sidecar alongside the Netdata agent in a Kubernetes cluster.

Example with a local config file:

```
$ docker run --rm -v /path/to/config.yaml:/config.yaml dhi.io/netdata-agent-sd:<tag> --config-file=/config.yaml
```

Example with a Kubernetes ConfigMap:

```
$ docker run --rm \
    -e MY_POD_NAMESPACE=default \
    dhi.io/netdata-agent-sd:<tag> --config-map=netdata-sd-config:config.yml
```
...
| Entry point        | Docker Hardened Images may have different entry points than images such as Docker Official Images. This image installs `agent_sd` at `/usr/local/bin/agent_sd` and provides a compatibility symlink at `/app/agent_sd` to match the upstream image entrypoint. |
...
1. Install additional packages
   ...
   For Alpine-based images, you can use `apk` to install packages.
