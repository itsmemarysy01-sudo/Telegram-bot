## About Calico Pod2Daemon FlexVol

Calico Pod2Daemon FlexVol is a FlexVolume driver installer used by Calico to set up secure connections from Kubernetes
pods to local daemons using Unix domain sockets. The Tigera Operator deploys this image as an init container on Calico
node pods to install the FlexVolume driver on each Kubernetes node.
