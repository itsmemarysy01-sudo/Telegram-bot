...
The Docker CLI image provides the complete Docker command-line interface for managing containers and images. To use it,
you need to mount the Docker socket from your host:

```console
$ docker run --rm -v /var/run/docker.sock:/var/run/docker.sock dhi.io/docker:<tag> ps
```

This runs `docker ps` inside the container while connecting to the Docker daemon on your host.

## Common Docker CLI use cases

### Manage containers from within a container

You can use the Docker CLI image to manage Docker containers from within another container, which is useful in CI/CD
pipelines:

```console
$ docker run --rm -v /var/run/docker.sock:/var/run/docker.sock dhi.io/docker:<tag> ps -a
```

### Build Docker images in CI/CD

Mount your project directory and use the Docker CLI to build images:
...
$ docker run --rm \
  ...
  -v $(pwd):/workspace \
  -w /workspace \
  dhi.io/docker:<tag> build -t myapp:latest .
```

### Use Docker Compose

The Docker CLI image includes Docker Compose for managing multi-container applications:
...
$ docker run --rm \
  ...
  -v $(pwd):/workspace \
  -w /workspace \
  dhi.io/docker:<tag> compose up -d
```

### Use Docker Buildx for multi-platform builds

The image includes Docker Buildx for advanced build scenarios:
...
$ docker run --rm \
  ...
  -v $(pwd):/workspace \
  -w /workspace \
  dhi.io/docker:<tag> buildx build --platform linux/amd64,linux/arm64 -t myapp:latest .
```

### Scan images with Docker Scout

Use Docker Scout to analyze images for vulnerabilities:

```console
$ docker run --rm \
  -v /var/run/docker.sock:/var/run/docker.sock \
  dhi.io/docker:<tag> scout cves myapp:latest
```

### Connect to a remote Docker daemon

Set environment variables to connect to a remote Docker daemon:
...
The Docker CLI image is commonly used as a base image for CI/CD containers:

```dockerfile
FROM dhi.io/docker:<tag>
