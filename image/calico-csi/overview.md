...
Calico CSI is a Container Storage Interface (CSI) driver used by Calico to set up secure connections from Kubernetes
pods to local daemons using Unix domain sockets. The Tigera Operator deploys this image as a DaemonSet container on each
Kubernetes node alongside the node-driver-registrar sidecar.
