...
### What's included in this Calico Whisker Backend Hardened Image

- Whisker Backend – HTTP API service that proxies flow telemetry from Goldmane for the Whisker UI

## Deploy Whisker Backend in Kubernetes

First follow the
[authentication instructions for DHI in Kubernetes](https://docs.docker.com/dhi/how-to/use/#authentication).

Whisker Backend runs alongside the Whisker UI when Calico observability is enabled. It exposes HTTP endpoints that
transform network flow data from Goldmane for display in the Whisker frontend. This image is not a standalone
application and is intended to run as part of a full Calico installation.
...
Whisker Backend image to use Docker Hardened Images via the Installation custom resource.

Whisker Backend requires Calico v3.30 or later and a Tigera Operator chart that includes the Whisker component.
...
**Important**: The Installation CR image configuration applies to **all** Calico component images, not just Whisker
Backend. The operator constructs image references using this format:
...
this configuration is in place. If you only have the Calico Whisker Backend DHI image available, other Calico components
may fail to start with `ImagePullBackOff` until their DHI images are also available in your registry.
