...
## Start a Linstor Affinity Controller instance

The LINSTOR Affinity Controller is designed to run as a Kubernetes deployment. The recommended deployment method is via
Helm chart:

```bash
helm repo add piraeus-charts https://piraeus.io/helm-charts/
helm install linstor-affinity-controller piraeus-charts/linstor-affinity-controller \
  --set image.repository=dhi.io/linstor-affinity-controller \
  --set image.tag=<tag> \
  --set linstor.endpoint=<your_linstor_endpoint>
```

To run the image directly (for testing purposes):

```bash
docker run --rm dhi.io/linstor-affinity-controller:<tag> --help
```

### Configuration

For details on available configuration options, including Helm chart values and controller flags, see the
...
**Available image tags for linstor-affinity-controller:**
...
- Use `dhi.io/linstor-affinity-controller:<version>` for standard production deployments
